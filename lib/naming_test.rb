...
module NamingTest
  ...
  def test_if_module_name(input)
    if %w[def = class == += -=].any? { |word| input.include? word }
      false
    elsif ['module '].any? { |word| input.strip[0, 7].include? word }
      true
    else
      false
    end
  ...
  def test_if_module_pascal_case(input)
    strip_input = input.strip.split('')
    only_name = strip_input[7, strip_input.size].to_a
    if only_name.any? { |letter| letter == '_' }
      false
    elsif only_name[0].to_s == only_name[0].to_s.upcase && only_name[0].to_s != only_name[0].to_s.downcase
      true
